 ...
 * @author dungdunght
 */
public class GameMenu extends engine.GameObject {
    ...
    public void update(long elapsedTime) {
       ...
       if (keyPressed(KeyEvent.VK_ENTER)) 
            if (menu_number==1)
                 {
                    getParent().setNextGameID(1);
                    finish();
                 }
            else{
                    getParent().setNextGameID(2);
                    finish();
            }
    ...
    public void render(java.awt.Graphics2D g2d) {
        Graphics2D engine_g2d = new Graphics2D(g2d);
        background.render(engine_g2d);
        g2d.setFont(new Font("Arial", Font.BOLD, 20));
        g2d.setColor(Color.WHITE);
        g2d.drawString("Press up,down to choose menu and press enter; press F1 to see help" , 30,30);
    }    
